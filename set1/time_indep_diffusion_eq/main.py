...
class laplace_solver:
    ...
    def __init__(self, N, tolerance=5E-5, objects=None):
        """
    ...
    def update_c(self, j, i):
        ...
        if self.objects:
            if (j, i) in self.objects:
                return 0
        if i == 0:
            value = self.grid[j, i+1] + self.grid[j, -2] + self.grid[j+1, i] + self.grid[j-1, i]
        elif i == len(self.grid) - 1:
            value = self.grid[j, 1] + self.grid[j, i-1] + self.grid[j+1, i] + self.grid[j-1, i]
        else:
